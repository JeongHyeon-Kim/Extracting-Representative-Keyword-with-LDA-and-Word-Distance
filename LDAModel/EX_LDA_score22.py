...
for exnum in range(0,10):
	sql = "SELECT * FROM Test4 limit 500 "
...
#		print(documentTopic)
		...
		ldashow = lda.show_topics(num_topics=24,num_words=10, formatted=False)
...
#						print("재검색")
						...
						for locsen in senword:
							sql9="select Word, 0.9*%s*(1/log(abs(Loc_in_sen-%s)+1)) as value from (select * from WordDistance where Sentence=%s and Articlenumber=%s) as B where Loc_in_sen != %s";
	...
	#					print(wordscore)
						...
						sql10="select Word, 0.1*%s*(1/log(abs(Loc_in_sen-%s)+1)) as value from (select * from WordDistance where Sentence=%s and Articlenumber=%s) as B where Loc_in_sen != %s ";
