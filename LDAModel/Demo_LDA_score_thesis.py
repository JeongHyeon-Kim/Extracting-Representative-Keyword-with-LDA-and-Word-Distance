...
sql = "select * from thesis where code =10 limit 6"
...
for row in rows:
	...
	# 사전 불러오고 Lda 모델 불러오기
	lda_model_path = "/home/ice-kms/LDAModel/Thesis20_LDA.lda"
	lda = LdaModel.load(lda_model_path)

	dictionary_path= "/home/ice-kms/LDAModel/Thesis20_LDADic.dict"
	dictionary = corpora.Dictionary.load(dictionary_path)

	document3 = row['Abstract']
	document2 = row['Conclusion']
	document = ''
...
#	print(documentTopic)
	...
	ldashow = lda.show_topics(num_topics=20,num_words=20, formatted=False)
#	print(ldashow)
	#초기화 부분	
	dicko = []
	clean_model=[]
...
#	print(row['ArticleNumber'])
	...
	#wordscore=[]
	# 모든 글자를 dict형태로 구성
	sql6 = "select Word from thesisWD where Number = %s" % (row['Number'])
	...
	# 문장의 개수 받아오기
	sql7 = "select Sentence from thesisWD where Number = %s group by Sentence" % (row['Number'])
...
#	print(LDAkeyword)
	...
	for sen in senNumber:
		...
		for word in LDAkeyword:
			...
			sql8="select Loc_in_sen from thesisWD where Number=%s and Sentence=%s and Word=%s";
			...
			if sen >1 and checksss == 0:
				for word in LDAkeyword:
#					print("재검색")
					sql8="select Loc_in_sen from thesisWD where Number=%s and Sentence=%s and Word like %s";
					cursor8.execute(sql8,(row['Number'],1,'%'+word+'%'))
					senword = cursor8.fetchall()

					for locsen in senword:
						sql9="select Word, 0.9*(1/log(abs(Loc_in_sen-%s)+1)) as value from (select * from thesisWD where Sentence=%s and Number=%s) as B where Loc_in_sen != %s ";
...
#					print(wordscore)
					...
					sql10="select Word, 0.1*(1/log(abs(Loc_in_sen-%s)+1)) as value from (select * from thesisWD where Sentence=%s and Number=%s) as B where Loc_in_sen !=%s";
...
#	print(sortsco)
	...
	sql5 = "select Word, count(*) as cnt from thesisWD where Number= %s group by Word order by cnt desc limit 5" % (row['Number']) 
...
#		for Real in RealTag:
#			cursor13.execute("insert into CombineTagging (ArticleNumber,Word) values (%s,%s)",(row['ArticleNumber'],Real))
	#		db.commit()
