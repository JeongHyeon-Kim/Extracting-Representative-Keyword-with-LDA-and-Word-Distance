...
for test in rows_test:
	split = test.split()
	num = 0
	for word in split:
                pos = t.nouns(split[num])
                for word2 in pos:
	...
	ldatest.append(docRemovingStopWord)
	tokens_ko=[]
...
ldamodel = gensim.models.ldamodel.LdaModel(corpus,num_topics=20,id2word = dictionary, passes = 100,iterations=1000)
...
perplex = ldamodel.bound(corpus2)
print("Topic20")
print("Perplex")
print(perplex)
...
word_perplex = numpy.exp2(-perplex / sum(cnt for document in corpus2 for _, cnt in document))
print(word_perplex)
