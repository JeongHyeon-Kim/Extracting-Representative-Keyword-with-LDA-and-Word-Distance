...
for exnum in range(0,10):
	sql = "select * from Test4 limit 100"
...
#						print("재검색")
						...
						for locsen in senword:
							sql9="select Word, 0.9*%s as value from (select * from WordDistance where Sentence=%s and Articlenumber=%s) as B where Loc_in_sen != %s ";
							cursor9.execute(sql9,(wordW,1,row['ArticleNumber'],locsen['Loc_in_sen']))
	...
	#				print(wordW)
					...
					if sen == 1:
						sql9="select Word, 0.9*%s as value from (select * from WordDistance where Sentence=%s and Articlenumber=%s) as B where Loc_in_sen != %s ";
						cursor9.execute(sql9,(wordW,sen,row['ArticleNumber'],locsen['Loc_in_sen']))
						firstsen = cursor9.fetchall()
	#					print("현재문장 번호:1")
						#print(firstsen)
						for fs in firstsen:
	...
	#					print(wordscore)
						...
						sql10="select Word, 0.1*%s as value from (select * from WordDistance where Sentence=%s and Articlenumber=%s) as B where Loc_in_sen !=%s";
						cursor10.execute(sql10,(wordW,sen,row['ArticleNumber'],locsen['Loc_in_sen']))
...
print("문서 가중치 0.9, 0.1 / 단어거리를 모두 1 / 500개 기사")
print("LDA 점수")
print(fls/25000)
print("LDA 유사")
print(flss/25000)
