...
sql = "select * from Demo order by Number limit 1"
...
#	print(LDAkeyword)
	...
	#wordscore=[]
	# 모든 글자를 dict형태로 구성
	sql6 = "select Word from WordDistance where Articlenumber = %s" % (row['Number'])
	...
	# 문장의 개수 받아오기
	sql7 = "select Sentence from WordDistance where Articlenumber = %s group by Sentence" % (row['Number'])
...
#					print(wordscore)
	...
	sql5 = "select Word, count(*) as cnt from WordDistance where ArticleNumber= %s group by Word order by cnt desc limit 5" % (row['Number']) 
...
#	print(str(out))
	...
	try :
		...
		cursor11.execute("insert into LDA (ArticleNumber,Word1,Word2,Word3,Word4,Word5) values(%s,%s,%s,%s,%s,%s)",(row['Number'],LDAscore[0],LDAscore[1],LDAscore[2],LDAscore[3],LDAscore[4]))
		db.commit()
		cursor12.execute("insert into Model (ArticleNumber,Word1,Word2,Word3,Word4,Word5) values(%s,%s,%s,%s,%s,%s)",(row['Number'],jh_model[0],jh_model[1],jh_model[2],jh_model[3],jh_model[4]))
		db.commit()
