...
for exnum in range(0,10):
	sql = "SELECT * FROM ( select * from Test4 limit 500) as A where A.Articlenumber not in (select ArticleNumber from CombineTagging group by ArticleNumber having count(*) < 9)"
...
#			print("다 0점")
		...
		sql5 = "select Word, count(*) as cnt from WordDistance where ArticleNumber= %s group by Word order by cnt desc limit 7" % (row['ArticleNumber']) 
...
#		print(RealTag)
		...
		try :
			...
			LDAscore.append(LDAkeyword[6])
			LDAscore.append(LDAkeyword[7])			
			LDAscore.append(LDAkeyword[8])
			...
			jh_model.append(sortsco[6][0])			
			jh_model.append(sortsco[7][0])
			jh_model.append(sortsco[8][0])
